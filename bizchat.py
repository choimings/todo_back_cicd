...
# text_splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=200) # 분할 토큰 수, 오버웹 정도
...
# 체인을 생성합니다.
# RunnablePassthrough(): 데이터를 그대로 전달하는 역할, invoke 메서드를 통해 입력된 데이터를 반환
# StrOutputParser(): LLM이나 ChatModel에서 나오는 언어 모델의 출력을 문자열 형식으로 반환
rag_chain = (
